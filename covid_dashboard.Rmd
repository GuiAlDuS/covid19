...
fun_cantones_resumen <- function(fecha_hoy){
  URL_excel <- paste0("http://geovision.uned.ac.cr/oges/archivos_covid/", 
                      ...
                      substr(fecha_hoy, 6, 7), "_", substr(fecha_hoy, 9, 10),"_EXCEL_GENERAL.xlsx")
...
fun_cantones_recuperados <- function(fecha_hoy){
  fecha_ayer <- as.character(as.character(Sys.Date() - lubridate::days(1)))
  url_pdf <- paste0("http://geovision.uned.ac.cr/oges/archivos_covid/", 
                   substr(fecha_hoy, 6, 7), "_", substr(fecha_hoy, 9, 10), "/",
                   substr(fecha_hoy, 6, 7), "_", substr(fecha_hoy, 9, 10),"_COMUNIC.pdf")
  area <- list()
  area[[1]] <- c(3200, 61, 3952, 1170)
  t <- extract_tables(url_pdf,
                      guess = FALSE,
                      area = area,
                      output = "data.frame")
  recuperados_raw <- d2[[1]][c(6:nrow(d2[[1]])),]
  recuperados1 <- recuperados_raw[,1:2]
  recuperados2 <- recuperados_raw[, c(4,6)]
  setnames(recuperados1, c(1,2), c("canton", "recuperados"))
  setnames(recuperados2, c(1,2), c("canton", "recuperados"))
  recuperados <- rbind(recuperados1, recuperados2)
  recuperados <- as.data.table(recuperados)
  recuperados <- recuperados[canton != ''][
    , fecha := lubridate::ymd(fecha_hoy) - lubridate::days(1)]
  
  recuperados_historico <- fread(paste0("recuperados_", fecha_ayer, ".csv"))
  recuperados_actual <- rbindlist(list(recuperados, recuperados_historico))
  
  fwrite(recuperados_historico, paste0("recuperados_", fecha_hoy, ".csv"))
  
  return(recuperados_actual)
}
...
g2 <- ggplot(t_gt_diauno[num_dia <= max_dia_CR + 5 & tipo == "contagios"], 
...
```{r echo = FALSE, fig.align="center"}
g1 <- ggplot(t_cntrys[tipo == "decesos" & 
                        #fecha >= fecha_min & 
                        fecha >= dmy("08/03/2020") &
                        casos > 0], 
       aes(x = fecha, y = casos, group = pais, col = pais)) + 
...
g2 <- ggplot(t_gt_diauno[num_dia <= max_dia_CR + 5 & tipo == "decesos"], 
       aes(x = num_dia, y = casos, group = pais, col = pais, )) + 
  geom_line(linetype = 2, size = 0.5, alpha = 0.5) +
  scale_y_continuous() +
  scale_x_continuous(minor_breaks = NULL) + 
...
g4 <- ggplot(tb_general[tipo == "decesos" & 
                          #fecha >= fecha_min &
                          fecha >= dmy("08/03/2020")], 
       aes(x = fecha, y = casos_10k, group = pais, col = pais)) + 
  geom_line(alpha = 0.7) +
  ...
  theme_light() +
  labs(x = "",
       y = "Decesos por cada 100,000 habitantes",
       col = "",
       title = "",
       subtitle = "") +
  theme(axis.text.x=element_text(angle=60, hjust=1),
...
g4_g <- g4 + 
  geom_point_interactive(
    aes(tooltip = paste0(
      pais, "\n", 
      paste0(day(fecha), " de ", month(fecha, label=TRUE, abbr=FALSE)), "\n", 
      round(casos_10k,1)," decesos/100k hab."),
      data_id = ID),
    size = 0.5, alpha = 0.7)
...
```{r echo = FALSE, fig.align="center"}
g1 <- ggplot(t_cntrys[tipo == "recuperados" & 
                        #fecha >= fecha_min & 
                        fecha >= dmy("08/03/2020") &
                        casos > 0], 
       aes(x = fecha, y = casos, group = pais, col = pais)) + 
...
g2 <- ggplot(t_gt_diauno[num_dia <= max_dia_CR + 5 & tipo == "recuperados"], 
       aes(x = num_dia, y = casos, group = pais, col = pais, )) + 
  geom_line(linetype = 2, size = 0.5, alpha = 0.5) +
  scale_y_continuous() +
  scale_x_continuous(minor_breaks = NULL) + 
...
g4 <- ggplot(tb_general[tipo == "recuperados" & 
                          #fecha >= fecha_min &
                          fecha >= dmy("08/03/2020")], 
       aes(x = fecha, y = casos_10k, group = pais, col = pais)) + 
  geom_line(alpha = 0.7) +
  ...
  theme_light() +
  labs(x = "",
       y = "Recuperados por cada 100,000 hab.",
       col = "",
       title = "",
       subtitle = "") +
  theme(axis.text.x=element_text(angle=60, hjust=1),
...
g4_g <- g4 + 
  geom_point_interactive(
    aes(tooltip = paste0(
      pais, "\n", 
      paste0(day(fecha), " de ", month(fecha, label=TRUE, abbr=FALSE)), "\n", 
      round(casos_10k,1)," recuperados/100k hab."),
      data_id = ID),
    size = 0.5, alpha = 0.7)
