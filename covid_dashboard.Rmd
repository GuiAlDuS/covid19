...
dt_cantones <- fread(paste0(fecha_hoy, "_cantones.csv"))
...
cantones_tidy$canton <- factor(cantones_tidy$canton, levels=unique(cantones_tidy$canton))
...
provincias <- wiki_tbl_diaria_2[, c(1:7,17)]
...
CR_detalles <- wiki_tbl_diaria_2[, c(9,11:15, 17)]

for(col in c("Confirmed","Deaths", "Hospitalized","ICU", "Rec", "Rej. Tests"))
  set(CR_detalles, j = col, value = as.integer(CR_detalles[[col]]))

for (col in c("Confirmed", "Deaths", "Hospitalized", "ICU", "Rec", "Rej. Tests")
     ) CR_detalles[is.na(get(col)), (col) := 0]
...
CR_detalles_diario <- CR_detalles[
  , .(Fecha,
      ...
      Descart. = `Rej. Tests` - shift(`Rej. Tests`, fill = 0))
]
