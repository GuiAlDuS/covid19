...
date: "`r format(Sys.time(), '%I:%M:%S %p %d-%m-%Y')`"
author: gds506
output: 
  html_document
---
...
fun_cantones_resumen <- function(fecha_hoy){
  ...
  tbls <- lapply(URLs, fread)
  
  positivos <- as.data.table(lapply(tbls[[1]], iconv, from = "macroman", to = "UTF-8" ))[, tipo := "positivos"]
  recuperados <- as.data.table(lapply(tbls[[2]], iconv, from = "macroman", to = "UTF-8" ))[, tipo := "recuperados"]
  fallecidos <- as.data.table(lapply(tbls[[3]], iconv, from = "macroman", to = "UTF-8" ))[, tipo := "fallecidos"]
  general <- as.data.table(lapply(tbls[[4]], iconv, from = "macroman", to = "UTF-8" ))
...
cantones_mapa <- merge(cantones_simp, 
                       dcast(cantones_tidy,  fecha + provincia + canton ~ tipo , value.var = "casos")[
                         , ID := seq_len(.N)
                       ], by = c("canton", "provincia"))
...
g2_g <- ggplot(t_gt_diauno[num_dia <= max_dia_CR + 5], 
  ...
  geom_point_interactive(
    ...
    size = 0.5, alpha = 0.7) +
  scale_y_continuous() +
  scale_x_continuous(minor_breaks = NULL, limits = c(15, NA)) + 
...
v_resumenDiario_CR <- ggplot(
  CR_detalles[ variable %in% c("positivos", "recuperados" , "fallecidos", "activos", "descartados")], 
...
* Los casos activos se calculan como: $activos = positivos - fallecidos - recuperados$
...
cant_geo <- cantones_mapa[
  cantones_mapa$fecha == fecha_hoy & 
    !is.na(cantones_mapa$positivos) & 
    cantones_mapa$positivos > 0, ]
...
### Evolución de casos activos por provincia
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}
g_prov <- ggplot(provincias, aes(x = fecha, y = positivos, fill = provincia)) +
  geom_col_interactive(
...
cant_heatmap <- as.data.table(cantones_mapa)
setorder(cant_heatmap, -canton, fecha)
cant_heatmap<- cant_heatmap[, nuevos_positivos := positivos - shift(positivos, fill = 0), by = canton
                            ][
  nuevos_positivos < 0, nuevos_positivos := 0
]

cant_heatmap$canton <- factor(cant_heatmap$canton, levels=unique(cant_heatmap$canton))

g_cant_1 <- ggplot(cant_heatmap, 
       aes(x = fecha, y = canton, fill = positivos)) + 
...
g_cant_1_g <- g_cant_1 + 
  geom_tile_interactive(
    aes(tooltip = paste0(
      ...
      positivos," casos totales", "\n",
      nuevos_positivos," casos nuevos"),
      data_id = ID), 
    width=1, height=.8)
