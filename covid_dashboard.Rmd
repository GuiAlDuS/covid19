...
fun_cantones_resumen <- function(fecha_hoy){
  ...
  cantones_recuperados <- fun_recuperados(fecha_hoy)
  cantones_datos <- merge(cantones_datos, cantones_recuperados, by = c("canton", "fecha"), all.x = TRUE)
  cantones_datos[, `:=`(activos = fifelse(is.na(recuperados), casos, casos - recuperados))]
...
fun_recuperados <- function(fecha_hoy){
  URL_excel_recup <- paste0("http://geovision.uned.ac.cr/oges/archivos_covid/", 
                      substr(fecha_hoy, 6, 7), "_", substr(fecha_hoy, 9, 10), "/",
                      substr(fecha_hoy, 6, 7), "_", substr(fecha_hoy, 9, 10),"_EXCEL_RECUP.xlsx")
  
  GET(URL_excel_recup, write_disk(tf <- tempfile(fileext = ".xlsx")))
  sheets <- excel_sheets(tf)
  
  cantones_d_vec <- like(sheets, "^\\d")
  
  cantones_recuperados_list <- lapply(sheets[cantones_d_vec], 
                                function(X) read_excel(
                                  tf, sheet = X, range = "B5:C86", col_names = c("canton", "recuperados")))
  names(cantones_recuperados_list) <- sheets[cantones_d_vec]
  
  cantones_recuperados <- rbindlist(cantones_recuperados_list, idcol = "fecha")
  cantones_recuperados[, fecha := lubridate::ymd(
    paste(
      year(Sys.Date()),
      substr(fecha,1, 2), 
      substr(fecha,4, 5),
      sep = "/"))]
  cantones_recuperados <- cantones_recuperados[recuperados > 0]
  
  #falta incluir diferencias entre fechas
  
  return(cantones_recuperados)
}
...
max_dia_CR <- max(t_gt_diauno[`Country/Region` == "Costa Rica" & tipo == "Confirmados", num_dia])
...
provincias = merge(cantones_tidy, cantones_simp, by = "canton")[
  , .(casos = sum(casos),
      ...
      activos = sum(activos)), by = c("provincia", "fecha")
][
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align="center"}
##Agregar los activos último día al siguiente paso
library(ggsflabel)
...
g_mapa <- ggplot() +
  ...
  geom_sf_text_repel(data = provincias_mapa, 
