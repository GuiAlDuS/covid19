...
fun_cantones_resumen <- function(fecha_hoy){
  ...
  csv_encoding <- "WINDOWS-1252" #"macroman"
  positivos <- as.data.table(lapply(tbls[[1]], iconv, from = csv_encoding, to = "UTF-8" ))[, tipo := "positivos"]
  recuperados <- as.data.table(lapply(tbls[[2]], iconv, from = csv_encoding, to = "UTF-8" ))[, tipo := "recuperados"]
  fallecidos <- as.data.table(lapply(tbls[[3]], iconv, from = csv_encoding, to = "UTF-8" ))[, tipo := "fallecidos"]
